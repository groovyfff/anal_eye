...
class ExternalMarketsService:
    """Сервис традиционных рынков: сканирование, запись в БД, публикация в RabbitMQ."""
    ...
    def __init__(self, settings: dict[str, Any]) -> None:
        ...
        self.history_depth = self._clamp_int_setting(
            raw_value=settings.get('history_depth', 200),
            min_value=2,
            max_value=200,
            setting_name='history_depth',
        )
        ...
        # yfinance отдаёт котировки с задержкой ~15с; для publish используем честный ts из провайдера
        self.yahoo_quote_freshness_ms = int(settings.get('yahoo_quote_freshness_ms', 1800000))
        self.price_fetch_workers = max(1, int(settings.get('price_fetch_workers', 8)))
        self.scan_workers = self._clamp_int_setting(
            raw_value=settings.get('scan_workers', 4),
            min_value=1,
            max_value=64,
            setting_name='scan_workers',
        )
        ...
        feature_cfg = {
            'breakout_lookback': trigger_cfg.get('price_breakout', {}).get('lookback_periods', 20),
            'volume_window': trigger_cfg.get('volume_spike', {}).get('window', 20),
            'ema_fast_period': ema_cfg.get('fast_period', 8),
            'ema_slow_period': ema_cfg.get('slow_period', 21),
        }
        ...
        self.scan_candidates = [
            ...
            if item['asset_class'] in {'stock', 'metal', 'forex'} and not item.get('use_for_correlation', False)
        ]
        self.broadcast_items = list(self.scan_candidates)
    ...
    def _build_collector(self, provider_cfg: dict[str, Any]) -> BaseCollector:
        ...
        api_key = provider_cfg.get('api_key', '') or os.environ.get('DATA_PROVIDER_API_KEY', '')
        request_delay_s = float(provider_cfg.get('request_delay_s', 0.25))
        raw_quote_delay = provider_cfg.get('quote_request_delay_s')
        quote_request_delay_s = None if raw_quote_delay is None else float(raw_quote_delay)
        max_retries = self._clamp_int_setting(
            raw_value=provider_cfg.get('max_retries', 5),
            min_value=1,
            max_value=5,
            setting_name='data_provider.max_retries',
        )
        ...
        if provider == 'polygon':
            return PolygonCollector(
                ...
                quote_request_delay_s=quote_request_delay_s,
    ...
    async def run(self) -> None:
        ...
        for attempt in range(1, self.rabbit_connect_retries + 1):
            ...
            if connected:
                break
            self.logger.warning(
                'RabbitMQ unavailable, retry %s/%s in %.1fs',
                attempt,
                self.rabbit_connect_retries,
                self.rabbit_connect_retry_delay_s,
                extra={'symbol': '-'},
            )
        ...
        self.logger.info(
            '[external-markets] Service started: scan=%s broadcast=%s (yfinance quote granularity ~15s)',
            len(self.scan_candidates),
            len(self.broadcast_items),
            extra={'symbol': '-'},
        )
    ...
    async def _scan_once(self) -> None:
        ...
        try:
            if self.reference_sp500 and stock_session_open and self._has_stock_candidates:
                benchmark_close = await self._fetch_close_series(
                    symbol=self.reference_sp500,
        ...
        try:
            if self.reference_dxy and metal_session_open and self._has_metal_candidates:
                dxy_close = await self._fetch_close_series(
                    symbol=self.reference_dxy,
                    timeframe=self.main_timeframe,
                    limit=self.history_depth,
                )
        except Exception as exc:
    ...
    async def _scan_symbol(
        ...
        try:
            ...
            indicator_df = self.feature_generator.compute_indicators(
                frame=candle_frame,
                asset_class=asset_class,
                benchmark_close=benchmark_close if asset_class == 'stock' else None,
                dxy_close=dxy_close if asset_class == 'metal' else None,
            )
            ...
            if len(analysis_df) < 2:
                return
            features = self.feature_generator.build_feature_payload(
                indicator_df=analysis_df,
                asset_class=asset_class,
                bid=quote.bid,
                ask=quote.ask,
            )
            ...
            self._has_required_asset_specific_features(symbol=symbol, asset_class=asset_class, features=features)
            ...
            payload = self._build_candidate_payload(
                symbol=symbol,
                asset_class=asset_class,
                display_name=item.get('name', symbol),
                features=features,
                trigger_result=trigger_result,
                indicator_df=analysis_df,
                patterns_payload=pattern_detection.patterns_payload,
                composite_score=score,
                signal_timestamp=signal_timestamp,
            )
            db_id = await self._persist_candidate(payload)
            if db_id is None:
                self.logger.error('Skipping publish: failed to persist signal_feature_logs row', extra={'symbol': symbol})
                return
            payload['signal_log_db_id'] = db_id
            published = await self._publish_json(
                exchange_name=self.exchange_name,
                routing_key=self.candidate_routing_key,
                payload=payload,
            )
            ...
            self.logger.info('Published candidate (%s) db_id=%s', ','.join(trigger_result.reasons), db_id, extra={'symbol': symbol})
        except Exception as exc:
            self.logger.error('Scan failed: %s', exc, extra={'symbol': symbol})

    async def _persist_candidate(self, payload: dict[str, Any]) -> int | None:
        if not self.db_enabled:
            self.logger.warning('DB disabled — using placeholder signal_log_db_id=0', extra={'symbol': payload.get('symbol', '-')})
            return 0
        try:
            async for session in DatabaseManager.get_session():
                return await save_external_candidate_log(session, payload)
        except Exception as exc:
            self.logger.error('DB persist failed: %s', exc, extra={'symbol': payload.get('symbol', '-')})
            return None
        return None
    ...
    def _has_required_asset_specific_features(self, symbol: str, asset_class: str, features: dict[str, Any]) -> bool:
        if asset_class == 'stock' and features.get('sp500_correlation') is None:
            self._warn_missing_asset_feature_once(symbol=symbol, feature_name='sp500_correlation')
        elif asset_class == 'metal' and features.get('dxy_correlation') is None:
            self._warn_missing_asset_feature_once(symbol=symbol, feature_name='dxy_correlation')
        elif asset_class == 'forex' and features.get('bid_ask_spread_pips') is None:
    ...
    def _warn_missing_asset_feature_once(self, symbol: str, feature_name: str) -> None:
        ...
        self.logger.warning(
            'Missing asset-specific feature %s; publishing candidate with null value',
            feature_name,
            extra={'symbol': symbol},
        )
    ...
    async def _broadcast_once(self) -> None:
        now_utc = dt.datetime.now(tz=dt.timezone.utc)
        active_items = [
            item for item in self.broadcast_items if self.market_hours.is_market_open(asset_class=item['asset_class'], now_utc=now_utc)
        ]
        if not active_items:
            return
        stale_symbols = [
            item['symbol'] for item in active_items if not self._is_quote_fresh(self._quote_cache.get(item['symbol']), now_utc)
        ]
        ...
        for item in active_items:
            ...
            payload = {
                ...
                'bid': quote.bid,
                'ask': quote.ask,
                'timestamp': publish_timestamp.isoformat().replace('+00:00', 'Z'),
                'ts': publish_ts_ms,
            }
            try:
    ...
    async def _publish_json(self, exchange_name: str, routing_key: str, payload: dict[str, Any]) -> bool:
        ...
        if routing_key == self.candidate_routing_key:
            validation_errors = validate_candidate_payload(payload, require_db_id=True)
        elif routing_key == self.price_routing_key:
            threshold = self.quote_freshness_ms
            if isinstance(self.collector, YahooFinanceCollector):
                threshold = self.yahoo_quote_freshness_ms
            validation_errors = validate_live_payload(
                payload,
                now_utc=dt.datetime.now(tz=dt.timezone.utc),
        ...
        if validation_errors:
            self.logger.error(
                'Skipping publish: invalid payload (%s)',
                '; '.join(validation_errors),
                extra={'symbol': str(payload.get('symbol', '-'))},
            )
            return False
        ...
        if self.use_thread_pool:
            try:
                ...
                if not self._thread_pool_fallback_warned:
                    self.logger.warning(
                        'Thread pool unavailable, falling back to inline blocking calls: %s',
                        exc,
                        extra={'symbol': '-'},
                    )
    ...
    def _build_candidate_payload(
        ...
        return {
            ...
            'trigger_reason': trigger_result.reasons[0],
            'trigger_reasons': list(trigger_result.reasons),
            'heuristic_signal_consensus': trigger_result.heuristic_signal_consensus,
            'features': features,
            'indicators': trigger_result.indicators,
            'patterns': patterns_payload,
            'historical_snapshots': self.feature_generator.build_historical_snapshots(indicator_df, count=3),
            'historical_ohlcv': self._build_historical_ohlcv(indicator_df, count=64),
            'composite_score': round(composite_score, 4),
            'entry_price_suggestion': 'market',
            'signal_log_db_id': None,
        }

    @staticmethod
    def _build_historical_ohlcv(indicator_df: Any, count: int = 64) -> list[dict[str, Any]]:
        if indicator_df is None or count <= 0:
            return []
    ...
    def _clamp_int_setting(self, raw_value: Any, min_value: int, max_value: int, setting_name: str) -> int:
        ...
        if clamped != numeric:
            self.logger.warning(
                'Setting %s=%s out of range, clamped to %s (allowed %s..%s)',
                setting_name,
                raw_value,
                clamped,
                min_value,
