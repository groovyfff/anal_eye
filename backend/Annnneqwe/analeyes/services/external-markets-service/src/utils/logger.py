...
class _JsonFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if not hasattr(record, 'symbol'):
            record.symbol = '-'
        payload: dict[str, Any] = {'timestamp': dt.datetime.fromtimestamp(record.created, tz=dt.timezone.utc).isoformat().replace('+00:00', 'Z'), 'level': record.levelname, 'logger': record.name, 'service_name': str(record.service_name), 'symbol': str(record.symbol), 'message': record.getMessage()}
        if record.exc_info:
...
def setup_logging(config: dict[str, Any]) -> None:
    ...
    service_name = config.get('service_name', 'external-markets-service')
    json_logs = bool(config.get('json', False))
    configured_format = str(config.get('format', '[%(service_name)s][%(symbol)s] %(asctime)s %(levelname)s %(name)s: %(message)s'))
...
def with_symbol(logger: logging.Logger, symbol: str) -> logging.LoggerAdapter[Any]:
