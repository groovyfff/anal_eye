...
def test_publish_json_skips_stale_live_payload() -> None:
    ...
    service.quote_freshness_ms = 4500
...
def test_scan_passes_pattern_score_and_payload() -> None:
    ...
    assert published[0]['name'] == 'Apple'
    assert 'historical_ohlcv' in published[0]
    assert len(published[0]['historical_ohlcv']) == 2


def test_build_candidate_payload_includes_display_name_and_ohlcv_window() -> None:
    service = ExternalMarketsService(_settings())
    frame = pd.DataFrame(
        {
    ...
    trigger_result = TriggerResult(
        triggered=True,
        reasons=['EMA_CROSSOVER_BULLISH'],
        heuristic_signal_consensus='LONG',
        indicators={'consensus': 'BULLISH', 'consensus_strength': 1.0, 'signals': []},
    )
    payload = service._build_candidate_payload(
        symbol='AAPL',
        asset_class='stock',
        display_name='Apple Inc.',
        features={'current_price': 102.5, 'rsi': 55.0, 'macd_hist': 0.2},
        trigger_result=trigger_result,
        indicator_df=frame,
        patterns_payload={'consensus': 'BULLISH', 'consensus_strength': 0.5, 'detected_patterns_info': []},
        composite_score=0.85,
        signal_timestamp=dt.datetime(2026, 1, 1, tzinfo=dt.timezone.utc),
    )
    assert payload['name'] == 'Apple Inc.'
    assert payload['signal_log_db_id'] is None
    assert len(payload['historical_snapshots']) == 3
    assert len(payload['historical_ohlcv']) == 3
    assert payload['historical_ohlcv'][0]['open'] == 100.0
    assert payload['historical_ohlcv'][0]['timestamp'].endswith('Z')
