...
class MarketHours:
    """Единый источник истины по торговым сессиям (используется tracker и external-markets)."""

    def __init__(self, config: dict | None = None) -> None:
        normalized = dict(config or {})
        normalized['metal_breaks_utc'] = self._validate_metal_breaks(normalized.get('metal_breaks_utc'))
        #ЕСЛИ ЧТО УБРАТЬ НАХУЙ
        allowed_keys = MarketHoursConfig.__annotations__.keys()
        filtered = {k: v for k, v in normalized.items() if k in allowed_keys}

        self.config = MarketHoursConfig(**filtered)
    ...
    def _is_stock_session_open(self, now_utc: dt.datetime) -> bool:
        ...
        if local_time.weekday() >= 5:
            return False
        use_fixed_utc_window = (
            bool(self.config.use_fixed_utc_window)
            and bool(self.config.stock_open_utc)
            and bool(self.config.stock_close_utc)
            and not self.config.pre_market_enabled
            and not self.config.after_hours_enabled
        )
    ...
    def _validate_metal_breaks(raw_value: object) -> list[str]:
        ...
        for raw in raw_value:
            if not isinstance(raw, str) or '-' not in raw:
                raise ValueError(f"Invalid metal break range '{raw}'")
